...
def get_onnx_model(model_checkpoint):
    device = torch.device("cuda:0")
    model = ORTModelForCausalLM.from_pretrained("optimum/gpt2").to(device)
    tokenizer = AutoTokenizer.from_pretrained(model_checkpoint)
    return (model, tokenizer)


def benchmark(seq_len, model, tokenizer, device, frame, iterations=200, num_beam=1, model_id="gpt2"):
    # prepare data
    payload = {
        "input_ids": torch.randint(low=0, high=tokenizer.vocab_size - 1, size=(seq_len,), dtype=torch.int32).unsqueeze(0).to(device),
...
for seq_len in seq_lengths:
    ...
    res.to_pickle(f"t4_res_ort_gpt2_beam5_{seq_len}.pkl")
